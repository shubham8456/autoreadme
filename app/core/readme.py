...
from app.ollama.service import ensure_model, generate_markdown, OllamaServiceError
...
def generate_readme(repo_path: Path, model: str, output_path: Path) -> None:
    try:
        ...
        print(f"\n[autoreadme] ERROR: {exc}", flush=True)
        raise SystemExit(1)
    except Exception as exc:
        print(f"\n[autoreadme] ERROR: Unexpected failure: {exc}", flush=True)
        raise SystemExit(1)
